...
export default async function IntegrationsPage() {
  ...
  const rows: Array<{ name: string; detail: string; status: "ok" | "off" | "later"; note?: string }> = [
    ...
    { name: "Apify (lead kaynakları)", detail: e.APIFY_TOKEN ? "Token tanımlı" : "Token yok", status: "later", note: "Faz 2" },
    { name: "E-posta sağlayıcı", detail: e.EMAIL_PROVIDER ?? "Tanımlı değil", status: "later", note: "Faz 3 — SMTP / Resend / Brevo / SendGrid" },
